...
Requirements:
TODO: Importing of a WF in U,V, verification of spherical conditions for the script.
TODO: Implememt a PANDYA transformation for particle-hole matrix elements
TODO: Implement the different DD matrix elements, for spherical symmetry but 
...
from bokeh.client import states
...
def particleHoleToNormalME_PandyaTransformation(hamil_ph):
    """
    Transform ph-hamiltonian into normal m.e.
    """
    hamil_J = {}
...
class RearrangementTBMESpherical_Runner():
    ...
    def run(self):
        ...
        # self._calculateDDMatrixElements(True)
    ...
    def _setUpHamiltoniannStatesAndAngularFunctions(self):
        ...
        """
        # TODO: 
        _LIST_qqnn = [self._ph_sort_states, self._ph_sort_pn_dir]
        zeros_ = [0.0 for t in range(6)]
        
        for bra, ket in set(self._ph_sort_states + self._ph_sort_pn_dir):
        ...
        for bra, ket in self._ph_sort_pn_exch_F:
            Jmin,  Jmax,  parOK  = self._getJRangeForBraKet(bra, ket)
            if not parOK: continue
        ...
        for bra, ket in self._ph_sort_pn_exch_P:
            Jmin,  Jmax,  parOK  = self._getJRangeForBraKet(bra, ket)
            if not parOK: continue
    ...
    def _setUpDensityMatrices(cls, dim_, u0_mat, v0_mat):
        ...
        for a, st_a in enumerate(cls.sh_states):
            ...
            cls.U_sh[ 1][st_a] = U[k, k]
            cls.U_sh[-1][st_a] = U[k+spO2, k+spO2]
            
            k2 = k + sh_o.j
            cls.V_sh[ 1][st_a] = V[k, k2]
            cls.V_sh[-1][st_a] = V[k+spO2, k2+spO2]
    ...
    #     """
    #
    #     phase_coeff = 1 + ((-1)**((self.ket.j1 + self.ket.j2)//2 + self.J + self.T))
    #     if abs(phase_coeff) < self.NULL_TOLERANCE: 
    #         return (True, 0.0)
    #
    ...
    #             recoupling *= (2*self.S + 1)*(2*self.L + 1)
    #             recoupling *= phase_coeff
    #
    ...
    def _calculateDDMatrixElements(self, pnpn_elements=False):
        """
        ...
        if pnpn_elements:
            t_range = (1, 4)
            ph_list_qqnn = self._2b_sort_states # self._ph_sort_pn_dir
        else:
            t_range = (0, 2, 3, 5)
            ph_list_qqnn = self._2b_sort_states # self._ph_sort_states
        
        ## When pnpn_case:     ad_sh & cb_sh   -->  ac_sh, db_sh
        for states in ph_list_qqnn:
            ad_sh, cb_sh = states
        ...
        for i, states in enumerate(self._ph_sort_states):
            ...
            for J in range(Jmin, Jmax +1):
                for t in (0, 2, 3, 5):
                    ...
                    for i in range(3): 
                        v_dd_dir[i] *= norm_
                        self._saveDDMatrixElement(v_dd_dir[i], ad_sh, cb_sh)
                        v_dd_exc[i] *= norm_
                        self._saveDDMatrixElement(v_dd_exc[i], ad_sh, cb_sh_exch)
            ...
            for i, states in enumerate(list_to_read):
                ...
                for J in range(Jmin, Jmax +1):
                    for t in (1, 4):
                        ...
                        for i in range(3): 
                            v_dd[i] *= norm_
                            self._saveDDMatrixElement(v_dd[i], bra_sh, ket_sh)
    ...
    def _Pi_abcd_matrix_general(self, ab_sh, cd_sh, _ICASE=0):
        ...
        for ee in self.sh_states:
            ...
            self._saveDDMatrixElement (v_dd, ab_sh, cd_sh, fix_1 = ee)
        ...
        for ee in self.sh_states:
            ...
            for ff in self.sh_states:
                ...
                if   self.t_indx in (0, 2, 3, 5):
                    tts = (ta,td, tc,tb)
                    assert _ICASE == 0, "Invalid use of the Pi m.e. (pp-pp)= ICASE 0"
                elif self.t_indx in (1, 4):
                    tts = (ta,tc, td,tb)
                    assert _ICASE == 2, "Invalid use of the Pi m.e. (pn-pn) -> (pp-nn) ICASE 2, 3"
                else:
                    raise Exception("WTF t index you set?")
                ...
                for J in range(Jmin, Jmax +1):
                    for TT in (1, -1): ## q1 in F.9, its independent of abcd
                        ...
                        ## Save the DD-matrix element with the intermediate value for TESTING
                        self._saveDDMatrixElement (v_dd, ab_sh, cd_sh, fix_1 = ee, fix_2 = ff)
                        self._saveREAMatrixElement(v_dd, ab_sh, cd_sh)
    ...
    def _convertThePNPN_phMatrixElements(self):
        ...
        for t in (1, 4):
            ...
            for i, states in enumerate(self._ph_sort_pn_dir):
                ...
                for J in range(Jmin, Jmax +1):
                    ...
                    for II in range(4): 
                        ...
                        if II in (0, 3):
                            Jvals_hamil = self._dd_hamil[II][ac_sh][db_sh]
                        elif II == 1: ## Pi
                            Jvals_hamil = self._P_hamil[ac_sh][db_sh]
                        elif II == 2: ## Pi
                            Jvals_hamil = self._Z_hamil[ac_sh][db_sh]
            ...
            for i, states in enumerate(self._ph_sort_pn_exch_F):
                # print(i, states)
                ab_sh, dc_sh = states
                ad_sh, bc_sh = self._ph_sort_states[i] ## to save
                ...
                for J in range(Jmin, Jmax +1):
                    Jvals_hamil = self._dd_hamil[3][ab_sh][dc_sh]
                    
                    args = [*adbc_sh2, J, t, Jvals_hamil]
                    val_pnpn = self._convertPPNNtoPNPN_phme(*args, **kwargs)
                    self._dd_hamil[II][ad_sh][bc_sh][J][t] = val_pnpn
            
            ## exchanged P
            for i, states in enumerate(self._ph_sort_pn_exch_P):
                ca_sh, bd_sh = states
                cb_sh, ad_sh = self._ph_sort_states[i] ## to save
                ...
                for J in range(Jmin, Jmax +1):
                    Jvals_hamil = self._P_hamil[ca_sh][bd_sh]
    ...
    def _radial_integral_calculation(self, IIR):
        ...
        if IIR == 0:
            ...
            int_ = np.inner(radial,  
                            ...
                            np.inner(self._spatial_dens_alpM1[t], angular))
            ...
            int_ = np.inner(radial,  
                            ...
                            np.inner(self._spatial_dens_alpM2[t], angular))
    ...
    def _saveDDMatrixElement(self, v_dd, bra, ket, fix_1 = None, fix_2 = None):
        ...
        if not ket in self._dd_hamil[II][bra]:
            ...
            ## only J = 0
                ...
                self._dd_hamil[II][bra][ket] = {0 : deepcopy(dict_) ,}
            elif II == 2:
                dict_ = {}
                for ab_sh in self.sh_states_sorted:
                    dict_[ab_sh] = {}
        ...
        if   II in (0, 3):
            if not J in self._dd_hamil[II][bra][ket]:
                self._dd_hamil[II][bra][ket][J] = [0.0 for _ in range(6)]
            if II == 3: ## F-rea matrix elements have 3 terms
    ...
    def _exportHamilsDDTesting(self):
        ...
        with open(filenames_[II], 'w+') as f:
            f.write('\n'.join(lines))
    ...
    def _exportQHamiltonian(self):
        ...
        """
        for II in range(2):
            lines = [f'Q-matrix element DD+REA [ph={bool(II)}] A,Z={self.A:7.5f},{self.Z:7.5f}   '
                     f'B,ALP,x0,t3[{self.B_LEN:8.5f}, {self.ALPHA:8.5f}, {self.x0:8.5f}, {self.CONST_t0:8.5f}]']
            ...
            fn_ = "Qph_hamilJ.2b" if II == 0 else "Q_hamilJ.2b"
        ...
        if (a, b) in self._PiZeta_hamil[I2]:
            if (c, d) in self._PiZeta_hamil[I2][(a, b)]:
                if J in self._PiZeta_hamil[I2][(a, b)][(c, d)]:
                    val = self._PiZeta_hamil[I2][(a, b)][(c, d)][J][t]
        return val
    ...
    def _computeRearrangementMatrixElements(self):
        ...
        for i, states in enumerate(self._ph_sort_states):
            ...
            a_sh = self.sh_states_obj[a]
            b_sh = self.sh_states_obj[b]
            c_sh = self.sh_states_obj[c]
            d_sh = self.sh_states_obj[d]
...
if __name__ == '__main__':
    ...
    # RearrangementTBMESpherical_Runner.setInteractionParameters(**kwargs)
